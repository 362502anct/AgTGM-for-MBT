function [u,e,iter] = linearinterVcycle_pcg(A0,A1,n,b,tol,omega,nsize)
    ...
    Acell{1} = A;
    Pcell{1} = speye(n *l);
         Pcell{2} = linearinter_prolong(ncell(1), lcell(1));
    ...
    if level+1 > 1
        for i = 3:level+1
            Pcell{i} = linearinter_prolong(ncell(i - 1),  lcell(i - 1));
...
%             iter-1,e(iter));
 ...
 fprintf('\n PCG Toep Vcycle at step %1.0f, relative residual = %e',... 
            iter-1,e(iter));
 fprintf('the level of V-cycle is: %e' , level);
